...
def test_be_basis_ported(r: int, d: int) -> None:
...
def test_surjection_basis_ported(r: int, d: int) -> None:
...
def test_planar_surjection_basis_ported(r: int, d: int) -> None:
...
def test_planar_preserved_under_composition_last_input_ported(
...
def test_section_right_inverse_ported(s: Surjection.Element) -> None:
    sect = s.section()
...
@pytest.mark.parametrize("s", PLANAR_LARGE)
def test_section_planar_ported(s: Surjection.Element) -> None:
    r = s.arity()
    sect = s.section()
